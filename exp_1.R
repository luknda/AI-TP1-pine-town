...
run_experiment <- function(datasets_to_pred, filepath) {
  ...
  for (dtp in datasets_to_pred) {#recorre todos los datasets
    for (impute in c("Yes", "No")) {#The loop will run twice: once with impute set to "Yes" and once with impute set to "No".
      for (prop_NAs in seq(0.1, 0.9, 0.2)) { #proporcion de NA's que va a haber en el dataset
        ...
        if (impute == "Yes") {
          preprocess_control <- list(
            prop_NAs=prop_NAs,
            impute_NAs=TRUE,#llena los NA's
...
plot_gap_results <- function(filename_exp_results, filename_plot, width, height) {
  # Load experiment results
  exp_results <- read.table(filename_exp_results, header=TRUE, sep="\t")
  
  # Calculate GAP in AUC
  gap_data <- exp_results %>%
    group_by(dataset_name, prop_NAs, maxdepth, IMPUTED) %>%
    summarize(mean_auc = mean(auc), .groups = 'drop') %>%
    pivot_wider(names_from = IMPUTED, values_from = mean_auc) %>%
    mutate(GAP = No - Yes)
  
  # Create a ggplot object for the line plot of GAP
  g <- ggplot(gap_data, aes(x = maxdepth, y = GAP)) +
    geom_line() +
...
plot_exp_results( "./outputs/tables/exp_1.txt", "./outputs/plots/plot_exp_1.jpg", width=5, height=4)
plot_gap_results( "./outputs/tables/exp_1.txt", "./outputs/plots/plot_2_exp_1.jpg", width=5, height=4)
